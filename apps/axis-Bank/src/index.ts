...
app.post("/axisBank", async (req, res): Promise<any> => {
  ...
  try 
  {
      ...
      await axios.post("https://saumilbisht.in/bank-webhook/axisWebhook", { token, user_identifier: userId, amount });
...
app.post("/axisBankWithdrawl", async (req, res): Promise<any> => {
    ...
    try{
        ...
        await axios.post("https://saumilbisht.in/bank-webhook/axisWebWithdrawl", { token, user_identifier: userId, amount });
        console.log("webhook se aagye sb badhiya")
