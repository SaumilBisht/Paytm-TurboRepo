...
app.post("/axisBank", async (req, res): Promise<any> => {
  ...
  try 
  {
      ...
      await axios.post(`${webhookURL}/axisWebhook`, { token, user_identifier: userId, amount });
...
app.post("/axisBankWithdrawl", async (req, res): Promise<any> => {
    ...
    try{
        ...
        await axios.post(`${webhookURL}/axisWebWithdrawl`, { token, user_identifier: userId, amount });
        console.log("webhook se aagye sb badhiya")
